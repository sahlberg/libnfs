...
void pmap_connect_cb(struct rpc_context *rpc, int status, void *data _U_, void *private_data)
{
	...
	if (rpc_pmap2_null_async(rpc, pmap_null_cb, client) == NULL) {
...
int main(int argc _U_, char *argv[] _U_)
{
	...
	if (null2) {
		if (rpc_pmap2_null_async(rpc, pmap2_null_cb, &client) == NULL) {
	...
	if (dump2) {
		if (rpc_pmap2_dump_async(rpc, pmap2_dump_cb, &client) == NULL) {
	...
	if (null3) {
		if (rpc_pmap3_null_async(rpc, pmap3_null_cb, &client) == NULL) {
	...
	if (dump3) {
		if (rpc_pmap3_dump_async(rpc, pmap3_dump_cb, &client) == NULL) {
	...
	if (gettime3) {
		if (rpc_pmap3_gettime_async(rpc, pmap3_gettime_cb, &client) == NULL) {
	...
	if (u2t3) {
		if (rpc_pmap3_uaddr2taddr_async(rpc, u2t3string, pmap3_uaddr2taddr_cb, &client) == NULL) {
	...
	if (getaddr3) {
		...
		if (rpc_pmap3_getaddr_async(rpc, &map, pmap3_getaddr_cb, &client) == NULL) {
	...
	if (set2) {
		if (rpc_pmap2_set_async(rpc, set2prog, set2vers, set2prot, set2port, pmap2_set_cb, &client) == NULL) {
	...
	if (unset2) {
		if (rpc_pmap2_unset_async(rpc, unset2prog, unset2vers, unset2prot, unset2port, pmap2_unset_cb, &client) == NULL) {
	...
	if (set3) {
		...
		map.addr  = set3addr;
		map.owner = set3owner;
		if (rpc_pmap3_set_async(rpc, &map, pmap3_set_cb, &client) == NULL) {
	...
	if (unset3) {
		...
		if (rpc_pmap3_unset_async(rpc, &map, pmap3_unset_cb, &client) == NULL) {
