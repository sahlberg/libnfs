...
int process_server(struct nfs_context *nfs, const char *server) {
	struct exportnode *exports;
	struct exportnode *export;

	exports = mount_getexports(nfs, server);
	if (exports == NULL) {
...
int main(int argc, char *argv[])
{
	...
	if (discovery) {
		...
		if (url->server && !url->path) {
			ret = process_server(nfs, url->server);
			goto finished;
		}
		nfs_destroy_url(url);
	}
